...
const studioMembership = computed(() => userStore.userInfo?.studioMembership || null)
const hasFullStudioAccess = computed(() => {
  const roles = userStore.userInfo?.roles || []
  return roles.some((role) => role.roleCode === 'ROLE_MERCHANT' || role.roleCode === 'ROLE_ADMIN')
})
const canCreateDesign = computed(() => {
  return hasFullStudioAccess.value || studioMembership.value?.canCreateDesign !== false
})
...
.pricing-page {
  ...
  --studio-surface-soft: #edf3f7;
  --studio-surface-raised: #ffffff;
  --studio-border: rgba(15, 23, 42, 0.1);
  --studio-primary: #0f766e;
  --studio-primary-soft: rgba(15, 118, 110, 0.1);
  --studio-primary-border: rgba(15, 118, 110, 0.22);
  --studio-accent: #2563eb;
  --studio-danger: #dc2626;
  --studio-text: #10202b;
  --studio-text-muted: #5a6b78;
  --studio-text-subtle: #7a8a96;
  --studio-radius-md: 8px;
  --studio-radius-lg: 8px;
  --studio-shadow-sm: 0 18px 50px rgba(15, 23, 42, 0.08);
  min-height: 100vh;
