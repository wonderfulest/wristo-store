...
test('purchase options keep the bundle before the product on phones', async () => {
  ...
  assert.ok(source.indexOf('class="purchase-option-bundle"') < source.indexOf('class="purchase-option-product"'))
  assert.doesNotMatch(mobileCss, /\.purchase-option-product\s*\{[^}]*order:/s)
  assert.doesNotMatch(mobileCss, /\.purchase-option-bundle\s*\{[^}]*order:/s)
